...
impl<'a, V, I: 'a + Iterator<Item = u8> + DoubleEndedIterator> Entry<'a, V, I> {
    ...
    where
        F: FnOnce() -> V,
    {
        if self.cursor.length != (u8::From(self.key.size_hint()).ok().unwrap()) {
            //expantion
        } else {
            let (node_box, node_body_v) = NodeBox::new_path(self.key.to_vec().drain(0..), f);
            let (header, body) = self.cursor.child.deref_mut().unwrap();
            body.update(self.key, node_box);
        }
    }
