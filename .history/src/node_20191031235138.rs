...
impl<V> NodeBox<V> {
    ...
    /// leaf node's value.
    pub fn new_path<I, F>(key: I, f: F) -> (Self, *const V)  // returns NodeBox, and value inside f
    where
        ...
        F: FnOnce() -> V,
    {
        ...
        for chunk in chunks { //starts from [h,a]
            ...
            key = *unsafe { chunk.get_unchecked(0) }; //becomes 'h'
            node = parent;
        }
