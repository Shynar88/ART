...
impl<'a, V, I: 'a + Iterator<Item = u8> + DoubleEndedIterator> Entry<'a, V, I> {
    ...
    pub fn delete(mut self) -> Result<V, ()> {
        ...
        self.cursor.parent.unwrap().deref_mut().unwrap().1.left().unwrap().delete(self.cursor.index);
        let child = self.cursor.child;
        Ok(child.into_value()) // should be called on cursor.child which should be leaf; into_value handles dropping the leaf
    }
