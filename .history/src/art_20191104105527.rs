...
impl<'a, V, I: 'a + Iterator<Item = u8> + DoubleEndedIterator> Entry<'a, V, I> {
        ...
        F: FnOnce() -> V,
    {
        ...
        if self.cursor.length != (header.length() as u8) {
            ...
            let mut new_headerkey = vec![KEY_INVALID; cursor.length as usize];
            new_headerkey.clone_from_slice(&header.key()[0..usize::from(self.cursor.length)]);
            header.shrink_key(cursor.length);
            let mut old_parent = cursor.parent.unwrap();
            let mut new_parent = NodeBox::newi(NodeHeader::new(&new_headerkey).unwrap(), vec![(key, node)], 0);
            let old_child = old_parent.deref_mut().unwrap().1.left().unwrap().delete(cursor.index).ok().unwrap();
