...
impl<'a, V, I: 'a + Iterator<Item = u8> + DoubleEndedIterator> Entry<'a, V, I> {
    ...
    where
        F: FnOnce() -> V,
    {
        if self.cursor.length != (self.key.count() as u8) {
            //expantion
        } else {
            let (node_box, node_body_v) = NodeBox::new_path((unsafe {&mut *(self.key as *mut NodeBox<V>)}), f);
            //let (header, body) = self.cursor.child.deref_mut().unwrap();
            //body.left().unwrap().update(self.key, node_box);
        }
        unimplemented!()
    }
